...
exports.createRoommate = function(req, res, next) {
	...
	db.select("SELECT * FROM salesforce.Account WHERE (identification_number__c ='" + c + "' or passport_number__c = '" + c + "' or student_id__c='" + c + "') and (secondary__c = false or secondary__c in not null) and (room__c is not null or room__c ='')")
	.then(function(results) {
		console.log(results);
		console.log(p);
