		...
		results.on('end', function() {
			try
			{
				...
				.then(function(results) {
						...
						query += " UNION ALL ";
					    query += "SELECT caseid as id, subject||' ('||workordernumber||')' as name, 'clean' as type, 'วันที่: '||to_char(working_date__c, 'DD/MM/YYYY') as detail, status, to_char(createddate, 'DD/MM/YYYY') as created_date, to_char(createddate, 'HH24:MI') as created_time, createddate FROM salesforce.WorkOrder WHERE accountid='" + results[0].sfid + "' and status='Completed'";
