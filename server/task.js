		...
		results.on('end', function() {
			try
			{
				...
				.then(function(results) {
						...
						query += " UNION ALL ";
						query += "SELECT sfid as id, 'Invoice No. '||name, 'billing' as type, 'สิ้นสุดชำระวันที่:'||to_char(due_date__c AT TIME ZONE 'GTM', 'DD/MM/YYYY')||',\n จำนวนเงิน:'||coalesce(total_amount__c, 0) as detail||'บาท', '' as status, to_char(createddate + interval '7 hour', 'DD/MM/YYYY') as created_date, to_char(createddate + interval '7 hour', 'HH24:MI') as created_time, createddate FROM salesforce.Invoice__c WHERE Student_Name__c='" + results[0].sfid + "'";
						query += " UNION ALL ";
